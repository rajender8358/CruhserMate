...
const MaterialRate = require(path.join(modelsPath, 'MaterialRate'));
const TruckEntry = require(path.join(modelsPath, 'TruckEntry'));
...
const setupProduction = async () => {
  try {
    ...
    console.log(`  - ${sureshOrg.name}`);
    console.log(`  - ${rajOrg.name}`);

    console.log('\n👥 Users Created:');
    console.log("Suresh's Crusher:");
    console.log(
      `  - Owner: ${sureshOwner.username} (${sureshOwner.firstName} ${sureshOwner.lastName})`,
    );
    console.log(
      `  - User: ${sureshUser.username} (${sureshUser.firstName} ${sureshUser.lastName})`,
    );
    console.log("Raj's Crusher:");
    console.log(
      `  - Owner: ${rajOwner.username} (${rajOwner.firstName} ${rajOwner.lastName})`,
    );
    console.log(
      `  - User: ${rajUser.username} (${rajUser.firstName} ${rajUser.lastName})`,
    );

    console.log('\n💰 Material Rates Created:');
    console.log("Suresh's Crusher:");
    sureshRates.forEach(rate => {
      console.log(
        `  - ${rate.materialType}: ₹${rate.currentRate.toLocaleString(
          'en-IN',
        )}`,
      );
    });
    ...
    console.log("  - suresh_owner / password123 - Owner of Suresh's Crusher");
    console.log("  - raj_owner / password123 - Owner of Raj's Crusher");
    console.log("  - suresh_user / password123 - User of Suresh's Crusher");
    console.log("  - raj_user / password123 - User of Raj's Crusher");
